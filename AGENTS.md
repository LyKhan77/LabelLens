...
- **Dataset Manager** — Standalone `/datasets` page for multi-project dataset management with Inference-style header navigation, delete confirmation modals, real overlay paginated thumbnail gallery (25/page), centered modal review with granular overlay controls and image delete, Rapid Inference jobs with per-image progress, configurable frame sampling, and zip export.
