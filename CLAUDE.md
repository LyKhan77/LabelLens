...
**Being Developed:**
- Train Tune / Custom Model Reuse: bbox detection dataset versioning, visual version preview, safe unused-version delete, detection checkpoint validation, queueing, Standard vs High-Speed GPU policy, live websocket progress, metrics history, and model registry are implemented — custom trained models are not yet wired back into the existing `/workspace` inference flow
...
- Overall: Awaiting `models/yoloe-26l-seg.pt` and `models/yoloe-26l-seg-pf.pt` placement plus real YOLO detection training checkpoints / hardware validation on RTX 5080 devices with `CUDA_DEVICE_ORDER=PCI_BUS_ID`
