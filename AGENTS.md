...
- **Auto-Crop Objects** — Separate, mutually-exclusive companion to Auto-Labelling for detection/segmentation only. Crops each detected object's bbox from the raw full-res frame (lossless PNG, HD) and saves it as a raw unannotated image into a Target Dataset (existing, or New with selectable Task Type, default Classification) for later OK/NG annotation — defect-detection workflow (detect → inference → auto-crop → annotate classification → train classifier). Cropping is backend-side from raw frames across image/video/RTSP. Toggle in Workspace sidebar and the Test Model sidebar.
...
| Train Tune Workspace | 2026-05-22 – 2026-06-17 | Implemented task-selectable training builder, immutable Dataset Version snapshots with persisted training architecture config, preprocessing policy, augmentation controls, training preflight (with base-checkpoint auto-download and multi-GPU AutoBatch handling), job queueing, task-aware live metrics, explicit missing-checkpoint diagnostics, result pages, resume/recompute/delete, artifact testing, and model registry. | Detection, segmentation, pose, and single-label classification jobs can be configured, relaunched from Dataset Versions with their saved YOLO family/size/checkpoint/settings, monitored with task-matched metrics, failed clearly when Ultralytics saves no checkpoint, resumed, tested, and tracked as reusable model versions. |
