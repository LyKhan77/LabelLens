...
- **Train Tune Workspace** — Dedicated `/train-tune` page for immutable dataset versions from live dataset projects or exported zips, train/val/test split, preprocessing and augmentation presets, YOLO11/YOLO26 configuration, Standard vs High-Speed GPU modes, real-time progress, metrics history, and model version registry
...
│       ├── train-tune/      (TrainTunePage — dataset versions, training jobs, metrics, models)
...
**Being Developed:**
- Train Tune / Custom Model Reuse: Dataset versioning, queueing, Standard vs High-Speed GPU policy, live websocket progress, metrics history, and model registry are implemented — custom trained models are not yet wired back into the existing `/workspace` inference flow
...
- Overall: Awaiting `models/yoloe-26l-seg.pt` and `models/yoloe-26l-seg-pf.pt` placement plus real YOLO training checkpoints / hardware validation on RTX 5080 devices with `CUDA_DEVICE_ORDER=PCI_BUS_ID`
