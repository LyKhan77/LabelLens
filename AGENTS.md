...
- **Test Model Gallery** — Dedicated `/test` entry from the Mode Select landing page. Master-detail Model Version picker: searchable list (task badge, architecture, best metric, date) on the left, selected-version info on the right (task, architecture, class chips, mAP50/mAP50-95, source dataset version, created date), with a single "Test Model" CTA that opens the existing `/train-tune/test/:id` tester. Reuses the training model registry; no backend changes.
- **Auto-Crop Objects** — Separate, mutually-exclusive companion to Auto-Labelling for detection/segmentation only. Crops each detected object's bbox from the raw full-res frame (lossless PNG, HD) and saves it as a raw unannotated image into a Target Dataset (existing, or New with selectable Task Type, default Classification) for later OK/NG annotation — defect-detection workflow (detect → inference → auto-crop → annotate classification → train classifier). Cropping is backend-side from raw frames across image/video/RTSP. Toggle in Workspace sidebar and the Test Model sidebar. Also available as an **Auto-Crop mode in the Dataset Upload Data dialog**: upload images, pick a detection/segmentation model (Pre-trained YOLOE + class names, or a Trained registry model version), then a backend crop job infers each image, keeps only the HD crops, and removes the originals (`POST /datasets/{name}/crop-jobs`).
...
│       ├── test-model/      (TestModelGalleryPage — `/test` model-version picker)
...
├── CHANGELOG.md         (implemented codebase state and AI-agent change history)
├── PRD.md, DESIGN.md, AGENTS.md, CLAUDE.md, README.md
```

## Current State - ALWAYS Update this section based on Changes or Features Made

**Codebase Changelog and Agent Memory:** The detailed implemented codebase state, long-term cross-agent memory, decision log, validation gaps, and AI-agent change history live in [`CHANGELOG.md`](CHANGELOG.md). Update `CHANGELOG.md` for every implemented behavior, architecture, model/CV stack, workflow, or major documentation change. Keep this section as a pointer only; do not duplicate the changelog table here.
