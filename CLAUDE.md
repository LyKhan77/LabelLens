...
**Being Developed:**
- Train Tune / Custom Model Reuse: bbox detection stepper versioning, locked Dataset Version preview, safe unused-version delete, detection checkpoint validation, queueing, Standard vs High-Speed GPU policy, live websocket progress, compact metric trends, scrollable epoch history, and model registry are implemented — custom trained models are not yet wired back into the existing `/workspace` inference flow
